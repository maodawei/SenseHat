...
class report:
    ...
    def return_rows(file_name):
        ...
        try:
            if len(sensehat_data_results) == 1:
                # extract the date to a variable names row_date
                row_date = sensehat_data_results[0][0].split()[0]
                # extract the temperature to a variable names row_temp
                row_temp = sensehat_data_results[0][1]
                # extract the humidity to a variable names row_hum
                row_hum = sensehat_data_results[0][2]
                row_status = report.check_status(file_name, row_temp, row_temp,  row_hum, row_hum)
    ...
    def log_to_plot_csv(file_name):
        ...
        with open('plot_data.csv', mode='w') as csv_file:
            ...
            # iterate though the lists 'date' and 'status'
            for d, min_t, max_t, min_h, max_h in zip(date, min_temp_ls, max_temp_ls, min_hum_ls, max_hum_ls):
                # write each row of the list to the csv file
                csv_file.writerow([d, min_t, max_t, min_h, max_h])
